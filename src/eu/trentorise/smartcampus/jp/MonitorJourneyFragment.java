...
public class MonitorJourneyFragment extends PlanNewJourneyFragment {
	...
	public void onStart() {
		...
		// ((EditText)
		// getView().findViewById(R.id.name)).setText(params.getName());
		// ((CheckBox)
		// getView().findViewById(R.id.recur_monitor)).setChecked(params.isMonitor());
		// Spinner spinner = (Spinner) getView().findViewById(R.id.recurrence);
		// spinner.setAdapter(new ArrayAdapter<String>(getSherlockActivity(),
	...
	protected void setUpTimingControls() {
		...
		Date d = params.getData().getParameters().getFromDate() > 0 ? new Date(params.getData().getParameters().getFromDate())
				: newDate;
		Date today = new Date();
		if (d.before(today))
			d = today;
